...
Redis.prototype.sadd = function(key, val){
    ...
    // debug("sadd called");
    return new Promise((resolve, reject)=>{
        self.redis.sadd(key.toString(), val.toString(), (err,val)=>{
            if(err) reject(err);
            else resolve(val);
        }) ;
    });
...
Redis.prototype.set = function(key, val){
    ...
    // debug("set called");
    return new Promise((resolve, reject)=>{
        self.redis.set(key.toString(), val.toString(), (err, val)=>{
            if(err) reject(err);
            else resolve(val);
        }) ;
    });
...
Redis.prototype.smembers = function(key, val){
    var self = this;
    // debug("set called");
    return new Promise((resolve, reject)=>{
        self.redis.smembers(key.toString(), (err, val)=>{
            if(err) reject(err);
            else resolve(val);
        }) ;
    });
